...
if (@ARGV != 6) {
	print "$0 contig.len.txt SequenceIDs.txt  groups.txt gene_location Species1 Species2 \n\n" ;
...
open (IN, "$SequenceIDFile") or die "dadiaodiaois\n" ; 
while (<IN>) {
...
while (<IN>) {
	...
	if ( $gene_count{$spe1} == 1 && $gene_count{$spe2} == 1) {
	    ...
	    # To change orthofinder ID -> original ID
	    my $gene1 = $orthoIDtoGene{ $genes{$spe1} } ; 
	    my $gene2 = $orthoIDtoGene{ $genes{$spe2} } ; 
