...
foreach my $gene ( @ids ) {
	...
	#print OUTGFF "$id_chr{$gene}\twormbase\tmRNA\t$id_start{$gene}\t$id_end{$gene}\t.\t$id_strand{$gene}\t.\tID=$gene:mRNA\;Name=$gene:mRNA\;Parent=$gene\n" ;
        ...
        print OUTGFF "$id_chr{$gene}\twormbase\tgene\t$id_start{$gene}\t$id_end{$gene}\t.\t$id_strand{$gene}\t.\tID=$WBid\;Name=$WBid\n" ;
	print OUTGFF "$id_chr{$gene}\twormbase\tmRNA\t$id_start{$gene}\t$id_end{$gene}\t.\t$id_strand{$gene}\t.\tID=$WBid:mRNA\;Name=$WBid:mRNA\;Parent=$WBid\n" ;
	...
	foreach (@exons) {
	    my @exon = split /-/, $_ ; 
	    print OUTGFF "$id_chr{$gene}\twormbase\texon\t$exon[0]\t$exon[1]\t.\t$id_strand{$gene}\t.\tID=$WBid:exon:$exon_num\;" ; 
	    print OUTGFF "Parent=$WBid:mRNA\;\n" ; 
	    $exon_num++ ; 
	}
