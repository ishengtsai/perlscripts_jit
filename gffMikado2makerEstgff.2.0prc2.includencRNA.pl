...
while (<IN>) {
    ...
    #look for cdna_length and combined_cds_fraction
    # https://mikado.readthedocs.io/en/latest/Algorithms.html
    # actually it's column 16?
    #print "$r[11]\t$r[15]\n" ; 
...
while (<IN>) {
	...
	#maker compliant gff
	$previous_gene_isncRNA = 1 ;
	print "$r[0]\test2genome\texpressed_sequence_match\t$gene_start_end{$tID}{'start'}\t$gene_start_end{$tID}{'end'}\t$included_trans_score{$tID}\t$r[6]\t.\tID=$tID.mikado2est\;Name=$tID\n" ;

    }
    elsif ( $r[2] eq 'exon' && $previous_gene_isncRNA == 1) {
	#next ;
	if ( /Parent=(\S+)/ ) {
	    $tID = $1 ;
	    #print "$tID\n" ;
