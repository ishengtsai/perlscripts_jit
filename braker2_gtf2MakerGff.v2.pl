...
open OUT, ">", "$file.formaker.gff" or die "ooops\n" ;
...
while (<IN>) {
	...
	#next if $r[2] eq 'start_codon' ;
	#next if $r[2] eq 'stop_codon' ;
	#next if	$r[2] eq 'exon' ;
	#next if $r[2] eq 'transcript' ;

	next unless $r[2] eq 'CDS' ;
	next unless $r[1] eq 'AUGUSTUS' ; 

	my $braker_transcript_id = '' ; 
	my $id = '' ;
	
	if ( /transcript_id \"(\S+)\";/ ) {
	    #print "$_\n" ;
	    $braker_transcript_id = $1 ; 
	    #next ; 

	}
	
	if ( $braker_transcript_id =~ /t(\d+)$/ ) {
	    next if $1 > 1 ; 
	}

	
	if ( $transcript_correspond_ID{$braker_transcript_id} ) {
	    $id = $transcript_correspond_ID{$braker_transcript_id} ; 
	 
	}
	else {
	    $genecount++ ;
	    my $idnum = sprintf("%07d", $genecount * 100) ;
	    $id = "$species\_$idnum.1" ;
	    $transcript_correspond_ID{$braker_transcript_id} = $id ;
	    $gene_loc{$r[0]}{$id} = "$r[3]\t$r[4]" ;
	    $gene_strand{$id} = "$r[6]" ;
	}
	
	#print "$_\t$braker_transcript_id\t$id\n" ; 
...
foreach my $contig (@contig_order ) {
	...
	if ( $gene_intron_num{$gene} ) {
	
	    for (my $i = 1 ; $i < ($gene_intron_num{$gene}+1) ; $i++) {
		
		print OUT "$contig\tbraker1\tintrons\t$gene_introns{$gene}{$i}\t.\t$gene_strand{$gene}\t.\tID=$gene:intron:$i\;Parent=$gene:mRNA\;\n" ;
