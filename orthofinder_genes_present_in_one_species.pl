...
open SUMMARY_UPSET, ">", "$file.upset.txt" or die "doapdaosdoa\n" ; 
...
while (<IN>) {
	...
	for ( my $i = 1 ; $i < @r ; $i++ ) {
	    ...
	    if ( $r[$i] =~ /$species\|(\S+)/ ) {
		print UNIQUE_LIST "$1\t$cluster_name\n" ; 
	    }
	}
    ...
    print SUMMARY_UPSET "$cluster_name\t" ;
