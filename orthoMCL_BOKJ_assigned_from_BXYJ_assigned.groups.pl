...
if (@ARGV != 6) {
	print "$0 contig.len.txt assigned_file groups.txt gene_location Species1 Species2 \n\n" ;
...
open (IN, "$assignedFile") or die "dadiaodiaois\n" ; 
while (<IN>) {

    chomp; 
    if ( /(\S+)\s+(\S+)/ ) {
	#print "$1\t$2\n" ; 
	$assigned_scaffold{$1} = $2 ; 
...
open OUT4, ">", "$file.$spe1.$spe2.locationcomparison.perscaffTable" or die "ooooca can't create file!\n" ; 
...
#my @celscaffolds = qw / CHROMOSOME_I CHROMOSOME_II CHROMOSOME_III CHROMOSOME_IV CHROMOSOME_V CHROMOSOME_X / ; 

my @celscaffolds = qw / 1 2 3 4 5 X X3 / ; 
