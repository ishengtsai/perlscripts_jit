...
if ( -e "$lane_noPCR\_2.fastq" ) {
	...
	system_call("$mapsplitterbin --split 10000 --setup_mem $index_ram --combine_mem 20 --array_mem $mapping_ram --tmpdir tmp.$lane_noPCR -c finaljob.$job_number -r -o \" -i $upper -r 10 -x -y 0.7  \" -k 13 -s 6 --keep_raw_only smalt $ref SMALT.$lane_noPCR $lane_noPCR\_1.fastq $lane_noPCR\_2.fastq ");
    }
    else {
	system_call("$mapsplitterbin --split 10000 --setup_mem $index_ram --combine_mem 20 --array_mem $mapping_ram --tmpdir tmp.$lane_noPCR -c finaljob.$job_number -o \" -i $upper -r 10 -x -y 0.7 \" -k 13 -s 6 --keep_raw_only smalt $ref SMALT.$lane_noPCR $lane_noPCR\_1.fastq $lane_noPCR\_2.fastq ");
    }
    ...
    if ( $reversecomplement == 1) {
	system_call("$mapsplitterbin --split 10000 --setup_mem $index_ram --combine_mem 20 --array_mem $mapping_ram --tmpdir tmp.$lane_noPCR -c finaljob.$job_number -r -o \" -i $upper -r 10 -x -y 0.7 \" -k 13 -s 6 --keep_raw_only    smalt $ref SMALT.$lane_noPCR     $lane_noPCR\_1.fastq.gz $lane_noPCR\_2.fastq.gz ");   
    }
    else {
	system_call("$mapsplitterbin --split 10000 --setup_mem $index_ram --combine_mem 20 --array_mem $mapping_ram --tmpdir tmp.$lane_noPCR -c finaljob.$job_number -o \" -i $upper -r 10 -x -y 0.7 \" -k 13 -s 6 --keep_raw_only    smalt $ref SMALT.$lane_noPCR     $lane_noPCR\_1.fastq.gz $lane_noPCR\_2.fastq.gz ");   
    }
